...
float4 PSWireTerrainMain(VSTerrainOutput input) : SV_TARGET
{
...
float3 CubicBezierSum5x5(OutputPatch<HSOutput, 25> patch, float uB[5], float vB[5])
{
...
DSOutput DSTerrainTessMain(PatchTess patchTess, float2 uv : SV_DomainLocation, const OutputPatch<HSOutput, 25> patch)
{
    ...
    // 제어점이 한 행 당 5개이므로 4차 베지어 곡면을 만든다.
    ...
    // 4차 베지어 곡면 계산
    result.position = float4(CubicBezierSum5x5(patch, uB, vB), 1.0f);
