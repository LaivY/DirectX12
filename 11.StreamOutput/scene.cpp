...
{
	m_meshes["TANK"] = make_shared<Mesh>(device, commandList, sPATH("tank.obj"));
...
void Scene::CreateGameObjects(const ComPtr<ID3D12Device>& device, const ComPtr<ID3D12GraphicsCommandList>& commandList)
{
	...
	auto skybox{ make_unique<Skybox>(
		array{ m_meshes.at("SKYBOX_FRONT"), m_meshes.at("SKYBOX_LEFT"), m_meshes.at("SKYBOX_RIGHT"),
			   m_meshes.at("SKYBOX_BACK"), m_meshes.at("SKYBOX_TOP"), m_meshes.at("SKYBOX_BOT")},
		m_shaders.at("SKYBOX"),
		array{ m_textures.at("SKYBOX_FRONT"), m_textures.at("SKYBOX_LEFT"), m_textures.at("SKYBOX_RIGHT"),
			   m_textures.at("SKYBOX_BACK"), m_textures.at("SKYBOX_TOP"), m_textures.at("SKYBOX_BOT") }
	) };
	...
	auto terrain{ make_unique<HeightMapTerrain>(device, commandList, wPATH("HeightMap.raw"),
												m_shaders.at("TERRAINTESS"), m_textures.at("TERRAIN"), 257, 257, 12, 12, terrainScale) };
	terrain->SetPosition(XMFLOAT3{ -257.0f / 2.0f, 0.0f, -257.0f / 2.0f });
	m_terrains.push_back(move(terrain));

	// 실내 생성
	auto indoor{ make_unique<GameObject>() };
	indoor->SetPosition(XMFLOAT3{ 0.0f, 500.0f, 0.0f });
	indoor->SetMesh(m_meshes.at("INDOOR"));
	indoor->SetShader(m_shaders.at("TEXTURE"));
	indoor->SetTexture(m_textures.at("INDOOR"));
	m_gameObjects.push_back(move(indoor));

	// 거울 생성
	auto mirror{ make_unique<GameObject>() };
	mirror->SetPosition(XMFLOAT3{ 0.0f, 500.0f - 7.5f, 14.5f });
	mirror->SetMesh(m_meshes.at("MIRROR"));
	mirror->SetShader(m_shaders.at("BLENDINGDEPTH"));
	mirror->SetTexture(m_textures.at("MIRROR"));
	...
	for (int i = 0; i < 9; ++i)
	{
		...
		testCube->SetMesh(m_meshes.at("CUBE"));
		testCube->SetShader(m_shaders.at("TEXTURE"));
		testCube->SetTexture(m_textures.at("INDOOR"));
